...
import kotlin.Pair;
import mrapple100.rtsp.parser.AacParser;
import mrapple100.rtsp.parser.RtpParser;
import mrapple100.rtsp.parser.VideoRtpParser;
import mrapple100.utils.NetUtils;
import mrapple100.utils.VideoCodecUtils;
...
public class RtspClient {
    ...
    public interface RtspClientListener {
        ...
        void onRtspConnected(@NotNull SdpInfo sdpInfo);
        void onRtspVideoNalUnitReceived(InputStream inputStream,@NotNull byte[] data, int offset, int length, long timestamp,byte type);
        void onRtspAudioSampleReceived(@NotNull byte[] data, int offset, int length, long timestamp);
...
//      public @Nullable byte[] sei; // H.265 only
    ...
    private final @NotNull Socket rtspSocket;
    private final @NotNull String uriRtsp;
    private final @NotNull AtomicBoolean exitFlag;
    private final @NotNull RtspClientListener listener;
...
//  private boolean sendOptionsCommand;
    ...
    private RtspClient(@NotNull RtspClient.Builder builder) {
...
//          if (sendOptionsCommand) {
            ...
            if (status == 401) {
                ...
                if (digestRealmNonce == null) {
                    ...
                    // Digest auth
                    authToken = getDigestAuthHeader(username, password, "OPTIONS", uriRtsp, digestRealmNonce.getFirst(), digestRealmNonce.getSecond());
                }
                checkExitFlag(exitFlag);
...
// a=control:trackID=2
            ...
            if (status == 401) {
                ...
                if (digestRealmNonce == null) {
                    ...
                    // Digest auth
                    authToken = getDigestAuthHeader(username, password, "DESCRIBE", uriRtsp, digestRealmNonce.getFirst(), digestRealmNonce.getSecond());
                }
                checkExitFlag(exitFlag);
                sendDescribeCommand(outputStream, uriRtsp, cSeq.addAndGet(1), userAgent, authToken);
                status = readResponseStatusCode(inputStream);
                ...
                if (track != null) {
                    ...
                    if (digestRealmNonce != null)
                        authToken = getDigestAuthHeader(
                                ...
                                "SETUP",
                                uriRtspSetup,
                                digestRealmNonce.getFirst(),
                                digestRealmNonce.getSecond());
                    sendSetupCommand(
                            outputStream,
                            uriRtspSetup,
                            cSeq.addAndGet(1),
...
// Session: Mzk5MzY2MzUwMTg3NTc2Mzc5NQ;timeout=30
            checkExitFlag(exitFlag);
            if (digestRealmNonce != null)
                authToken = getDigestAuthHeader(username, password, "PLAY", uriRtsp /*?*/, digestRealmNonce.getFirst(), digestRealmNonce.getSecond());
            ...
            if (sdpInfo.videoTrack != null ||  sdpInfo.audioTrack != null) {
                if (digestRealmNonce != null)
                    authToken = getDigestAuthHeader(username, password, hasCapability(RTSP_CAPABILITY_GET_PARAMETER, capabilities) ? "GET_PARAMETER" : "OPTIONS", uriRtsp, digestRealmNonce.getFirst(), digestRealmNonce.getSecond());
              ...
              //  //  Log.d(TAG_DEBUG, "Sending keep-alive");
                ...
                try {
                    ...
                    if (hasCapability(RTSP_CAPABILITY_TEARDOWN, capabilities)) {
                        if (digestRealmNonce != null)
                            authToken = getDigestAuthHeader(username, password, "TEARDOWN", uriRtsp, digestRealmNonce.getFirst(), digestRealmNonce.getSecond());
    ...
    @Nullable
    private static String getUriForSetup(@NotNull String uriRtsp, @Nullable Track track) {
        if (track == null || track.request.isEmpty())
            return null;
    ...
    private static void checkExitFlag(@NotNull AtomicBoolean exitFlag) throws InterruptedException {
        if (exitFlag.get())
    ...
    private static void readRtpData(
            ...
            @NotNull SdpInfo sdpInfo,
            @NotNull AtomicBoolean exitFlag,
            @NotNull RtspClientListener listener,
            int keepAliveTimeout,
            @NotNull RtspClientKeepAliveListener keepAliveListener)
    throws IOException {
                    ...
                    switch (type) {
                        case VideoCodecUtils.NAL_SPS:
                            ...
                            if (nalUnit.length > 100)
                                listener.onRtspVideoNalUnitReceived(inputStream,nalUnit, 0, nalUnit.length, (long)(header.timeStamp * 11.111111),type);
                            break;
...
//                                listener.onRtspNalUnitReceived(nalUnitSppPpsIdr, 0, nalUnitSppPpsIdr.length, System.currentTimeMillis());
                                ...
                                listener.onRtspVideoNalUnitReceived(inputStream,nalUnitSppPps, 0, nalUnitSppPps.length, (long)(header.timeStamp * 11.111111),type);
...
//                            listener.onRtspNalUnitReceived(nalUnit, 0, nalUnit.length, System.currentTimeMillis() / 10);
    ...
    throws IOException {
        //if (debug)
        System.out.println( "sendOptionsCommand(request=\"" + request + "\", cSeq=" + cSeq + ")");
        System.out.println(userAgent);
    ...
    private int readResponseStatusCode(@NotNull InputStream inputStream) throws IOException {
...
//            }
    ...
    @NotNull
    private ArrayList<Pair<String, String>> readResponseHeaders(@NotNull InputStream inputStream) throws IOException {
        ArrayList<Pair<String, String>> headers = new ArrayList<>();
        String line;
        while (!exitFlag.get() && !((line = readLine(inputStream)).isEmpty())) {
            //if (debug)
              //  //  Log.d(TAG_DEBUG, "" + line);
            if (CRLF.equals(line)) {
                return headers;
                ...
                if (pairs.length == 2) {
                    headers.add(new Pair(pairs[0].trim(), pairs[1].trim()));
                }
            }
        }
        return headers;
     ...
     * @return array of 2 tracks. getKey() is video track, getValue() audio track.
     */
    @NotNull
    private static Track[] getTracksFromDescribeParams(@NotNull List<Pair<String, String>> params) {
        ...
        for (Pair<String, String> param: params) {
            switch (param.getFirst()) {
                case "m":
                    // m=video 0 RTP/AVP 96
                    if (param.getSecond().startsWith("video")) {
                        currentTrack = new VideoTrack();
                        tracks[0] = currentTrack;

                    // m=audio 0 RTP/AVP 97
                    } else if (param.getSecond().startsWith("audio")) {
                ...
                case "a":
                    // a=control:trackID=1
                    if (currentTrack != null) {
                        if (param.getSecond().startsWith("control:")) {
                            currentTrack.request = param.getSecond().substring(8);
                        ...
                        } else if (param.getSecond().startsWith("fmtp:")) {
                            // Video
                        ...
                        } else if (param.getSecond().startsWith("rtpmap:")) {
                            // Video
                            if (currentTrack instanceof VideoTrack) {
                                String[] values = param.getSecond().split( " ");
                                if (values.length > 1) {
...
// a=framesize:96 1080-1920
    ...
    // Pair getKey() - name, e.g. "a"; getValue() - value, e.g "cliprect:0,0,1920,1080"
    @NotNull
    private static List<Pair<String, String>> getDescribeParams(@NotNull String text) {
    ...
    @NotNull
    private static SdpInfo getSdpInfoFromDescribeParams(@NotNull List<Pair<String, String>> params) {
        SdpInfo sdpInfo = new SdpInfo();
        ...
        for (Pair<String, String> param : params) {
            switch (param.getFirst()) {
                case "s": sdpInfo.sessionName = param.getSecond(); break;
                case "i": sdpInfo.sessionDescription = param.getSecond(); break;
            }
        }
        return sdpInfo;
    }
    ...
    // a=fmtp:97 streamtype=5;profile-level-id=1;mode=AAC-hbr;sizelength=13;indexlength=3;indexdeltalength=3;config=1408
    @Nullable
    private static List<Pair<String, String>> getSdpAParams(@NotNull Pair<String, String> param) {
        if (param.getFirst().equals("a") && param.getSecond().startsWith("fmtp:")) { //
            String value = param.getSecond().substring(8).trim(); // fmtp can be '96' (2 chars) and '127' (3 chars)
            String[] paramsA = value.split( ";");
            // streamtype=5
            // profile-level-id=1
            // mode=AAC-hbr
    ...
    private static void updateVideoTrackFromDescribeParam(@NotNull VideoTrack videoTrack, @NotNull Pair<String, String> param) {
        // a=fmtp:96 packetization-mode=1;profile-level-id=42C028;sprop-parameter-sets=Z0LAKIyNQDwBEvLAPCIRqA==,aM48gA==;
        // a=fmtp:96 packetization-mode=1; profile-level-id=4D4029; sprop-parameter-sets=Z01AKZpmBkCb8uAtQEBAQXpw,aO48gA==
        ...
        if (params != null) {
            for (Pair<String, String> pair: params) {
                switch (pair.getFirst().toLowerCase()) {
                    case "sprop-parameter-sets":
                        {
                            String[] paramsSpsPps = pair.getSecond().split( ",");
                            if (paramsSpsPps.length > 1) {
                                byte[] sps = Base64.getDecoder().decode(paramsSpsPps[0]);
                                byte[] pps = Base64.getDecoder().decode(paramsSpsPps[1]);
    ...
    @NotNull
    private static byte[] getBytesFromHexString(@NotNull String config) {
        // "1210fff1" -> [12, 10, ff, f1]
        return new BigInteger(config ,16).toByteArray();
    }

    private static void updateAudioTrackFromDescribeParam(@NotNull AudioTrack audioTrack, @NotNull Pair<String, String> param) {
        // a=fmtp:96 streamtype=5; profile-level-id=14; mode=AAC-lbr; config=1388; sizeLength=6; indexLength=2; indexDeltaLength=2; constantDuration=1024; maxDisplacement=5
        // a=fmtp:97 streamtype=5;profile-level-id=1;mode=AAC-hbr;sizelength=13;indexlength=3;indexdeltalength=3;config=1408
        // a=fmtp:96 profile-level-id=1;mode=AAC-hbr;sizelength=13;indexlength=3;indexdeltalength=3;config=1210fff15081ffdffc
        List<Pair<String, String>> params = getSdpAParams(param);
        if (params != null) {
            for (Pair<String, String> pair: params) {
                switch (pair.getFirst().toLowerCase()) {
                    case "mode": audioTrack.mode = pair.getSecond(); break;
                    case "config": audioTrack.config = getBytesFromHexString(pair.getSecond()); break;
                }
            }
        }
    }

    private static int getHeaderContentLength(@NotNull ArrayList<Pair<String, String>> headers) {
    ...
    private static int getSupportedCapabilities(@NotNull ArrayList<Pair<String, String>> headers) {
        for (Pair<String, String> head: headers) {
            String h = head.getFirst().toLowerCase();
            // Public: OPTIONS, DESCRIBE, SETUP, PLAY, GET_PARAMETER, SET_PARAMETER, TEARDOWN
            if ("public".equals(h)) {
                int mask = 0;
                String[] tokens = head.getSecond().toLowerCase().split( ",");
    ...
    @Nullable
    private static Pair<String, String> getHeaderWwwAuthenticateDigestRealmAndNonce(@NotNull ArrayList<Pair<String, String>> headers) {
        for (Pair<String, String> head: headers) {
            String h = head.getFirst().toLowerCase();
            ...
            if ("www-authenticate".equals(h) && head.getSecond().toLowerCase().startsWith("digest")) {
                String v = head.getSecond().substring(7).trim();
                int begin, end;
                ...
                return new Pair(digestRealm, digestNonce);
            }
        }
        return null;
    }
    ...
    @Nullable
    private static String getHeaderWwwAuthenticateBasicRealm(@NotNull ArrayList<Pair<String, String>> headers) {
        for (Pair<String, String> head: headers) {
            // Session: ODgyODg3MjQ1MDczODk3NDk4Nw
            String h = head.getFirst().toLowerCase();
            String v = head.getSecond().toLowerCase();
            // WWW-Authenticate: Digest realm="AXIS_00408CEF081C", nonce="00054cecY7165349339ae05f7017797d6b0aaad38f6ff45", stale=FALSE
            // WWW-Authenticate: Basic realm="AXIS_00408CEF081C"
    ...
    private static String getBasicAuthHeader(@Nullable String username, @Nullable String password) {
        ...
        return "Basic " + new String(Base64.getEncoder().encode(auth.getBytes(StandardCharsets.ISO_8859_1)));
    }
...
//                    userName, digestRealm, digestNonce, digestUri, response);
    ...
    @NotNull
    private static String getHexStringFromBytes(@NotNull byte[] bytes) {
    ...
    @NotNull
    private static String readContentAsText(@NotNull InputStream inputStream, int length) throws IOException {
        if (length <= 0)
            return "";
    ...
    private static void shiftLeftArray(@NotNull byte[] array, int num) {
        // ABCDEF -> BCDEF
        if (num - 1 >= 0)
            System.arraycopy(array, 1, array, 0, num - 1);
    }

    private boolean readUntilBytesFound(@NotNull InputStream inputStream, @NotNull byte[] array) throws IOException {
...
//    private boolean readUntilByteFound(@NotNull InputStream inputStream, byte bt) throws IOException {
...
//    }
    ...
    private static int readData(@NotNull InputStream inputStream, @NotNull byte[] buffer, int offset, int length) throws IOException {
        //if (debug)
    ...
    private static void dumpHeaders(@NotNull ArrayList<Pair<String, String>> headers) {
        if (DEBUG) {
            for (Pair<String, String> head : headers) {
                //  Log.d(TAG, head.getFirst() + ": " + head.getSecond());
            }
        }
    }

    @Nullable
    private static String getHeader(@NotNull ArrayList<Pair<String, String>> headers, @NotNull String header) {
        for (Pair<String, String> head: headers) {
            // Session: ODgyODg3MjQ1MDczODk3NDk4Nw
            String h = head.getFirst().toLowerCase();
...
//      private boolean sendOptionsCommand = true;
        ...
        public Builder(
                @NotNull Socket rtspSocket,
                @NotNull String uriRtsp,
                @NotNull AtomicBoolean exitFlag,
                @NotNull RtspClientListener listener) {
...
class  LoggerOutputStream extends BufferedOutputStream {
    ...
    public  LoggerOutputStream(@NotNull OutputStream out) {
        super(out);
    }
